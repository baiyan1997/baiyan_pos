...
public class _Activity_2202_UI : ActivityUI
{
    //每日宝箱按钮
    ...
    public void InitRef()
    {
        ...
        _listView = ListView.Create<CumulativeRechargeItemServer>(_listViewRoot);
        ...
        _actInfo = (ActInfo_2202)ActivityManager.Instance.GetActivityInfo(_aid);
    }
    ...
    private void Refresh()
    {
        ...
        _vipPointNum.text = Lang.Get("活动期间充值氪晶得好礼!\n", currentRechargeNum);
        var tempActInfo = _actInfo.GetAct2202DetailInfo();
        for (int i = 1; i < tempActInfo.Count; i++)
        {
            _listView.AddItem<CumulativeRechargeItemServer>().Show(tempActInfo[i], currentRechargeNum, _actInfo.GetReward);
        }
    ...
    {
        _actInfo.GetReward(Cfg.Activity2202.GetDailyTreasureTid(), OnActGetRewardCB);
    }
...
public class CumulativeRechargeItemServer : ListItem
{
    ...
    {
        _goalDesc.text = Lang.Get(Cfg.Activity2202.GetDescriptionByTid(_tid));
        P_Item[] rewards = GlobalUtils.ParseItem(Cfg.Activity2202.GetRewardByTid(_tid));

        int num = 0;
        for (num = 0; num < rewards.Length; num++)
        {
            var _itemShow = ItemForShow.Create(rewards[num].Id, rewards[num].count);
            _rewards[num].rewardId = rewards[num].Id;
            _rewards[num].icon.SetActive(true);
            _itemShow.SetIcon(_rewards[num].rewardIcon);
            _rewards[num].rewardQue.color = _ColorConfig.GetQuaColorHSV(_itemShow.GetQua());
    ...
    public void Show(Act2202Detail stageGoal, int currentStageProgress, Action<int, Action> callback)
    {
        _tid = stageGoal.tid;
        _callback = callback;
        ...
        _progress.text = string.Format("（<Color=#00ff00ff>{0}</Color>/{1}）", stageGoal.do_number, Cfg.Activity2202.GetStageGoalByTid(_tid));
