...
public class ActInfo_2204 : ActivityInfo
{
    ...
    {
        int nRealDay = dayTid;
        if(nRealDay > DAYNODELIMIT)
        {
            nRealDay -= DAYNODELIMIT;
        }
        P_MissionData d;
        _dicDayInfo.TryGetValue(nRealDay, out d);
        if (d == null)
        {
            throw new KeyNotFoundException("Can't find P_MissionData, dayTid = " + dayTid);
        }

        if(dayTid > DAYNODELIMIT)
        {
            return (d.do_number == 1);
        }
        return (d.get_reward != 0);
    }
    //获取对应时间奖励
    public void GetAct2204Reward(int day, Action callback = null)
    {
        Rpc.SendWithTouchBlocking<P_ActCommonReward>("getAct2204Reward", Json.ToJsonString(day), data =>
        {
            //添加道具
